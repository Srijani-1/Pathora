...
import { 
  ...
  Globe, Cpu, ArrowRight, ChevronDown 
} from "lucide-react";
...
export default function WelcomePage({ onFinish }: { onFinish: () => void }) {
  ...
  const heroBlur = useTransform(scrollYProgress, [0, 0.1], ["0px", "20px"]);
  
  // SCROLL TO EXPLORE OPACITY
  const scrollPromptOpacity = useTransform(scrollYProgress, [0, 0.05], [1, 0]);

  // QUAD-COLOR TRANSITION: Blue -> Indigo -> Purple -> Dark Teal
    ...
    <div ref={containerRef} className="relative bg-[#020202] text-white selection:bg-blue-500 overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
        ...
        <motion.div 
          ...
          className="absolute top-[-15%] left-[-15%] w-[80%] h-[80%] rounded-full blur-[160px] transition-colors duration-1000" 
        />
        
        {/* Deep Space Particle Layer */}
        <div className="absolute inset-0 opacity-[0.2] bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:40px_40px]" />
        ...
        <div className="absolute inset-0 opacity-[0.15]">
          <svg width="100%" height="100%">
            <pattern id="master-grid" x="0" y="0" width="120" height="120" patternUnits="userSpaceOnUse">
              ...
              <path d="M 2 2 L 120 120 M 120 2 L 2 120" stroke="rgba(59, 130, 246, 0.1)" strokeWidth="0.5" fill="none" />
            </pattern>
      ...
      {/* NAV */}
      <nav className="fixed top-0 w-full z-[100] p-8 flex justify-between items-center backdrop-blur-xl bg-black/20 border-b border-white/5">
        <div className="flex items-center gap-4">
          <div className="w-11 h-11 bg-blue-600 rounded-xl flex items-center justify-center border border-blue-400/30 shadow-[0_0_20px_rgba(37,99,235,0.3)]">
      ...
      <section className="sticky top-0 h-screen flex flex-col items-center justify-center z-30 pointer-events-none">
        ...
        <motion.div 
          ...
          className="absolute bottom-12 flex flex-col items-center gap-4 text-zinc-600"
        >
      ...
      {/* SECTION 2: CTA */}
      <section className="relative h-screen z-40 flex flex-col items-center justify-center">
        <h2 className="text-6xl md:text-8xl font-black tracking-tighter mb-4 uppercase italic leading-[0.85] text-center px-6 mb-20">
          ...
          <span className="text-blue-600">Pathora is Clear.</span>
        </h2>
      ...
      <section className="relative z-40 py-48 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <BentoCard icon={<Target />} activeColor="text-blue-500" title="Vector Roadmap" desc="Visualize your journey across multi-dimensional skill vectors. We track the depth of your understanding." />
            <BentoCard icon={<Flame />} activeColor="text-orange-500" title="Momentum Loops" desc="Daily velocity tracking to keep your learning engine primed." />
            <BentoCard icon={<Clock />} activeColor="text-emerald-500" title="Time Synthesis" desc="Deep-focus analytics to maximize the ROI of your learning hours." />
            <BentoCard icon={<Trophy />} activeColor="text-white" title="Milestone Mastery" desc="Unlock industry-standard credentials verified by actual project execution data." />
          </div>
        </div>
      </section>

      {/* SECTION 4: PROTOCOL */}
      <section className="relative z-40 py-48 px-6 bg-[#020202]/50">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="grid lg:grid-cols-3 gap-24 relative">
             <div className="hidden lg:block absolute top-[48px] left-[15%] right-[15%] h-[1px] bg-white/5 -z-10">
                <AnimatePresence>
                  {activeStep !== null && (
                    <motion.div 
                      initial={{ opacity: 0, scaleX: 0 }} animate={{ opacity: 1, scaleX: 1 }} exit={{ opacity: 0, scaleX: 0 }}
                      className="absolute inset-0 bg-gradient-to-r from-blue-500 via-purple-400 to-teal-500 shadow-[0_0_20px_rgba(59,130,246,0.6)] origin-center"
                    />
                  )}
                </AnimatePresence>
             </div>
      ...
      <footer className="relative z-40 py-24 px-8 border-t border-white/5 text-center bg-black/80 backdrop-blur-3xl">
...
function BentoCard({ icon, title, desc, activeColor }: any) {
  return (
    <motion.div 
      whileHover="hover"
      className="relative p-10 rounded-[3rem] border border-white/10 bg-white/0 backdrop-blur-3xl flex flex-col justify-between transition-all duration-500 overflow-hidden min-h-[380px] group"
    >
      <div className="absolute inset-0 opacity-[0.3] pointer-events-none">
        <svg width="100%" height="100%">
          <pattern id="card-pattern" x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
            ...
            <line x1="0" y1="0" x2="60" y2="60" stroke="rgba(255,255,255,0.06)" strokeWidth="0.5" />
          </pattern>
          <rect width="100%" height="100%" fill="url(#card-pattern)" />
        </svg>
      </div>
      <motion.div variants={{ hover: { scale: 1.1 } }} className={cn("w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 relative z-10 transition-colors duration-500 text-zinc-800", `group-hover:${activeColor}`)}>
      ...
      <div className="relative z-10">
        ...
        <p className="opacity-40 group-hover:opacity-80 transition-opacity font-light text-sm leading-relaxed">{desc}</p>
      </div>
    </motion.div>
  );
}
...
function ProtocolStep({ num, title, desc, icon, onHover, onLeave }: any) {
  return (
    <motion.div onMouseEnter={onHover} onMouseLeave={onLeave} whileHover="hover" className="relative flex flex-col items-center group cursor-default">
      <div className="relative w-24 h-24 rounded-3xl bg-zinc-950 border border-white/5 flex items-center justify-center text-zinc-900 mb-8 transition-all duration-500 group-hover:text-blue-500 group-hover:border-blue-500 group-hover:shadow-[0_0_30px_rgba(59,130,246,0.3)]">
      ...
      <span className="text-xs font-black text-zinc-800 group-hover:text-blue-600 mb-4 tracking-[0.3em] uppercase transition-colors">{num}</span>
      <h4 className="text-2xl font-bold mb-4 tracking-tight uppercase italic group-hover:text-white transition-colors">{title}</h4>
      <p className="text-zinc-600 text-sm font-light leading-relaxed px-6 group-hover:text-zinc-400 transition-colors">{desc}</p>
    </motion.div>
  );
}